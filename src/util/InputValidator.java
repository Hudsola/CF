...
public class InputValidator {
    ...
    // -------------------------------------------------------------------------
    // Leitura com valor padrão (edição) — ENTER mantém o valor atual
    // -------------------------------------------------------------------------

    /** Lê texto opcional; ENTER mantém valorAtual. Não aceita string vazia como novo valor. */
    public String lerTextoComPadrao(String label, String valorAtual) {
        System.out.print(label + " [" + valorAtual + "] (ENTER para manter): ");
        String v = sc.nextLine().trim();
        return v.isEmpty() ? valorAtual : v;
    }

    /** Lê valor monetário opcional; ENTER mantém valorAtual. Repete em caso de número inválido. */
    public double lerValorComPadrao(String label, double valorAtual) {
        while (true) {
            System.out.printf("%s [R$ %.2f] (ENTER para manter): ", label, valorAtual);
            String linha = sc.nextLine().trim();
            if (linha.isEmpty()) return valorAtual;
            try {
                double v = Double.parseDouble(linha.replace(",", "."));
                if (v > 0) return v;
                erro("O valor deve ser maior que zero.");
            } catch (NumberFormatException e) {
                erro("Valor inválido. Use apenas números e vírgula/ponto decimal (ex: 1500,00).");
    ...
    /** Lê data opcional; ENTER mantém dataAtual. Repete em caso de data inválida. */
    public LocalDate lerDataComPadrao(String label, LocalDate dataAtual) {
        while (true) {
            System.out.print(label + " [" + dataAtual.format(FMT) + "] (ENTER para manter, ou dd/MM/yyyy): ");
            String linha = sc.nextLine().trim();
            if (linha.isEmpty()) return dataAtual;
    ...
    /** Lê inteiro opcional dentro de um intervalo; ENTER mantém valorAtual. */
    public int lerIntComPadrao(String label, int valorAtual, int min, int max) {
        while (true) {
            System.out.printf("%s [%d] (ENTER para manter, faixa %d-%d): ", label, valorAtual, min, max);
            String linha = sc.nextLine().trim();
            if (linha.isEmpty()) return valorAtual;
            try {
    ...
    /**
     * Seleciona uma conta opcionalmente; ENTER mantém contaAtual.
     * Exibe a lista apenas se o usuário quiser trocar.
     */
    public Conta selecionarContaComPadrao(List<Conta> contas, Conta contaAtual) {
        System.out.println("Conta atual: " + contaAtual);
        System.out.print("Trocar de conta? (S/N): ");
        boolean trocar = false;
        while (true) {
            String r = sc.nextLine().trim().toUpperCase();
            if (r.equals("S")) { trocar = true; break; }
            if (r.equals("N")) { trocar = false; break; }
            System.out.print("Digite S para Sim ou N para Não: ");
        }
        return trocar ? selecionarConta(contas) : contaAtual;
    }

    /**
     * Seleciona uma categoria opcionalmente; ENTER mantém categoriaAtual.
     */
    public Categoria selecionarCategoriaComPadrao(List<Categoria> categorias, Categoria categoriaAtual) {
        System.out.println("Categoria atual: " + categoriaAtual);
        System.out.print("Trocar de categoria? (S/N): ");
        boolean trocar = false;
        while (true) {
            String r = sc.nextLine().trim().toUpperCase();
            if (r.equals("S")) { trocar = true; break; }
        ...
        return trocar ? selecionarCategoria(categorias) : categoriaAtual;
    }
